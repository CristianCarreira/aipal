...
- `AIPAL_THREAD_MAX_CONTEXT_CHARS`: force thread rotation when the accumulated context (all prompts + responses in the thread) exceeds this many characters. Default: `40000` (~10K tokens). This is the primary safeguard against runaway token consumption — especially for cron jobs that execute repeatedly on the same thread. Set to `0` to disable. Both limits (turns and context size) are evaluated and whichever triggers first wins.
