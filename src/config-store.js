...
const VOICE_OVERRIDES_PATH = path.join(CONFIG_DIR, 'voice-overrides.json');
...
async function loadVoiceOverrides() {
  try {
    ...
    if (err && err.code === 'ENOENT') return new Map();
    console.warn('Failed to load voice-overrides.json:', err);
    return new Map();
  }
}

async function saveVoiceOverrides(overrides) {
  await fs.mkdir(CONFIG_DIR, { recursive: true });
  const obj = Object.fromEntries(overrides);
  const tmpPath = `${VOICE_OVERRIDES_PATH}.${randomUUID()}.tmp`;
  await fs.writeFile(tmpPath, JSON.stringify(obj, null, 2));
  await fs.rename(tmpPath, VOICE_OVERRIDES_PATH);
}
